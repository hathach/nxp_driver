...
static void I3C_SlaveTransferEDMACallback(edma_handle_t *dmaHandle, void *param, bool transferDone, uint32_t tcds)
{
    ...
    if (transferDone)
    {
        ...
        if (i3cHandle->txDmaHandle == dmaHandle)
        {
            ...
            {
                /* Send the last byte. */
                i3cHandle->base->SWDATABE = *(uint8_t *)(((uint32_t)(uint32_t *)i3cHandle->transfer.txData) + i3cHandle->transfer.txDataSize - 1U);
            }
        }
        else
        {
...
static void I3C_SlavePrepareTxEDMA(I3C_Type *base, i3c_slave_edma_handle_t *handle)
{
    ...
    else
    {
        ...
        EDMA_PrepareTransfer(&txConfig, xfer->txData, 1, (void *)txFifoBase, 1, 1, xfer->txDataSize - 1,
                             kEDMA_MemoryToPeripheral);
    }
